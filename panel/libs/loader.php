...
function getMessage($type,$remove = false) {
    if($type == 'all'){
        return count($_SESSION['message']) > 0;
    }
...
function getViewFile($view){
    $view = trim($view,"/");
    if(is_file(__DIR__."/../views/$view.phtml")){
        return __DIR__."/../views/$view.phtml";
    }else{
        return false;
    }
}
...
function getLayoutFile($layout) {
    ...
    if(is_file(__DIR__."/../views/$layout.phtml")){
        return __DIR__."/../views/$layout.phtml";
    }else{
        return false;
    }
...
function getTitle(){
    return "GiaPanel ".$_SESSION['title'];
}
